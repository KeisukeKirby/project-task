...
export function OverviewDashboard() {
  ...
  // Bottleneck: find which step has the most overdue or delayed tasks
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-surface-500 text-sm mt-1">{formatDate(new Date().toISOString())}</p>
        </div>
      </div>
              ...
              {users.map((user) => {
                ...
                return (
                  <div key={user.id} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-medium text-surface-700 w-16 truncate">{user.name}</span>
                    <div className="flex-1">
                      <div className="progress-bar">
                        <div className="progress-bar-fill bg-primary-500" style={{
                          width: `${Math.min((userTasks.length / Math.max(user.max_concurrent_tasks, 1)) * 100, 100)}%`
                        }} />
                      </div>
                    </div>
                    <span className="text-[10px] text-surface-500 w-12 text-right">{userTasks.length}/{user.max_concurrent_tasks}</span>
                    {userOverdue > 0 && (
                      <span className="text-[10px] font-bold text-danger">⚠ {userOverdue}</span>
                    )}
                  </div>
                );
              })}
